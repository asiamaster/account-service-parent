...
public class CardManageServiceImpl implements ICardManageService {
	...
	public void returnCard(CardRequestDto cardRequest) {
		...
		// 更新卡状态
		int update = userCardDao.updateStateById(userCardDo.getId(), CardStatus.RETURNED.getCode(),
				userCardDo.getVersion());
		if (update == 0) {
			throw new AccountBizException(ResultCode.DATA_ERROR, "退卡操作失败");
		}
		cardStorageService.activateCard(cardRequest.getCardNo(), cardRequest.getFirmId());
	}
	...
	public UserCardDo changeCard(CardRequestDto cardParam) {
		...
		// 新卡出库，原卡变为激活
		cardStorageService.activateCard(oldCard.getCardNo(), cardParam.getFirmId());
		cardStorageService.inUse(newCard.getCardNo(), cardParam.getFirmId());
		return newCard;
	}
	...
	private void validateCanChangeCard(CardAggregationWrapper wrapper, CardRequestDto cardParam) {
		...
		// 主卡换主卡，副卡换副卡
		CardStorageDo cardStorageDo = cardStorageService.getByCardNo(userCard.getCardNo(), cardParam.getFirmId());
